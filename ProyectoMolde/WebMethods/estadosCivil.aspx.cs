...
{
    public partial class estadosCivil : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static Result getListaEstadosCivil(int registroPartida, int totalAExtraer, int usuarioId)
        {
            ...
            List<EstadosCivilViewModel> lst = new List<EstadosCivilViewModel>();
            try
            {
                lst = EstadosCivilController.getListaEstadosCivil();
        ...
        public static Result eliminar(int id, int usuarioId)
        {
            ...
            try
            {
                return EstadosCivilController.eliminarEstadosCivil(id, usuarioId);
            }
            catch (Exception ex)
            {
                return new Result() { error = ex.Message, id = 0, tipoAlerta = "warning" };
